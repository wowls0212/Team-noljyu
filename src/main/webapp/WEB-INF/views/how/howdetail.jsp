        ...
        if (value == true) {
            $.ajax({
                ...
                success: function(bigo) {
                    if (bigo == "success") { 
                        alert("삭제가 완료되었습니다");
                        let url = 'howdetail?hownum=${dto.hownum}';
...
<c:forEach items="${list}" var="rr" varStatus="status">
	...
	<tr style="border-bottom: none;">
		...
		<input type="hidden" value="${rr.reviewnum}" id="reviewnum${status.index}"></th>
		...
		<!-- <a href="howrere?reviewnum=${rr.reviewnum}">댓글</a> -->
		</td>
	</tr>
	...
	<tr id="howreview3${status.index}" style="display: none;">		
    	...
    	<textarea mexlength="300" id="reveiwUpdate-content${status.index}" class="form-control" rows="2" cols="100" placeholder="댓글을 입력해 주세요">${rr.review}</textarea>
	...
	<tr style="border-left: none;border-right: none;border-bottom: none">
		...
		<c:if test="${paging.startPage!=1 }">
		   <a href="howdetail?hownum=${dto.hownum}&nowPage=${paging.startPage-1}&cntPerPage=${paging.cntPerPage}"></a>
		</c:if>   
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p"> 
		   <c:choose>
		      ...
		      <c:when test="${p != paging.nowPage}">
		         <a href="howdetail?hownum=${dto.hownum}&nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
		      </c:when>   
		   </c:choose>
		</c:forEach>      
		
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="howdetail?hownum=${dto.hownum}&nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage }">  </a>
		</c:if>   
	   </td>
	</tr>
</table>
